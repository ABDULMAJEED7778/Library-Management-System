...
def reserve(request):
    if request.method == 'POST':
        ...
        if user_reservations.count() >= MAX_ALLOWED_RESERVATIONS:
            messages.error(
                request, 'You have the maximum number of reservations and cannot make any more')
            return redirect(book)
        else:
            # check to see if user has book on loan
            record_queryset = Record.objects.filter(
                user_id=user_id, book_title=book.title, date_returned=None)
            if record_queryset:
                ...
                if user_reservations.filter(book=book):
                    messages.error(
                        request, 'You have already reserved this book')
                    return redirect(book)
                else:
                    ...
                    if did_change_status:
                        ...
                        ), can_collect=True, reservation_expiry=timezone.now()+timezone.timedelta(days=7))
                        reservation.save()
                        send_mail(
                        ...
                        reservation = Reservation(
                            book=book, user_id=user_id, reservation_date=timezone.now())
                        reservation.save()

                    messages.success(
                        request, 'You have now reserved this book')
                    # or /book/+book.id if id was a string
                    return redirect(book)
