...
def users(request):
    ...
    if 'keyword' in request.GET:
        ...
        if keyword.isnumeric():
            users_memb_num = users.filter(
                profile__memb_num__icontains=int(keyword))
        else:
            users_memb_num = None
        if users_memb_num:
            users = (users_username | users_last_name |
                     users_memb_num).distinct()
        else:
...
def loan_books(request, user_id):
    ...
    active_loans_count = Record.objects.filter(
        user_id=user_id, date_returned=None).count()
    if active_loans_count >= MAX_ALLOWED_LOANS:
        messages.error(
            request, 'User has maximum number of active loans allowed (' + str(active_loans_count) + ')')
        return redirect('user', looked_up_user.id)
    ...
    if user_reservations:
        ...
        for reservation in user_reservations:
            ...
            book_reservations = Reservation.objects.filter(
                book=reservation.book).order_by('reservation_date')
            ...
            reserved_instances_for_book = BookInstance2.objects.filter(
                book=reservation.book, status='r')
...
def execute(request):
    if request.method == 'POST':
        ...
        active_loans_count = Record.objects.filter(
            user_id=user_id, date_returned__isnull=True).count()
                ...
                if user_reservations:
                    for reservation in user_reservations:
                        if reservation.book.id == an_instance.book.id:
                            reservation.delete()
            messages.success(
                request, 'The books have been successfully loaned out')
            return redirect('user', user.id)
...
def return_book(record):
    ...
    waiting_reservations = Reservation.objects.filter(
        book=instance.book, can_collect=False).order_by('reservation_date')
