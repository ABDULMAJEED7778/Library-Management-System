...
def export_to_csv(modeladmin, request, queryset):
    ...
    writer.writerow(['title', 'author', 'isbn10',
                    'isbn13', 'category', 'tags'])
...
class BookAdmin(admin.ModelAdmin):
    ...
    # cos this is redundant - should really remove from model
    exclude = ['tags']
    search_fields = ('title',)
    list_per_page = 50
...
class BookInlineForSeries(admin.TabularInline):
    model = Book
    extra = 0
    exclude = ['other_authors', 'summary', 'tags', 'user_tags', 'secondary_tags', 'language',
               'publish_date', 'photo', 'is_featured', 'users_like', 'series_num', 'category']
...
class BookInlineForCategory(admin.TabularInline):
    ...
    extra = 0
    exclude = ['other_authors', 'summary', 'tags', 'user_tags', 'secondary_tags', 'language',
               'publish_date', 'photo', 'is_featured', 'users_like', 'series_num', 'series']
    readonly_fields = ('author', 'title')
