    ...
    if 'term' in request.GET:
        ...
        if term != 'the' and len(term) > 2:
            qs = Book.objects.filter(
                title__icontains=term, instances__isnull=False).distinct()[:20]
            titles = list()
            for bk in qs:
...
def book(request, book_id, slug):
    ...
    if book.series:
        other_books_in_series = Book.objects.filter(
            series=book.series).exclude(id=book.id).order_by('series_num')
...
def tag_search(request):
    ...
    context = {
        ...
        'language_choices': language_choices,
        'books': paged_books,
        # this is for getting the search terms to show up in the search results page
        'values': request.GET,
    ...
    if request.method == 'POST':
        if 'title' in request.POST:
            ...
            # else if an author has been entered in
                ...
                if len(names) == 1:
                    messages.error(
                        request, 'Need to enter a first and last name for author')
                    return redirect('add_book')
                else:
                    first_name = names[0]
                    del names[0]
                    ...
                    try:
                        ...
                        if middle_name:
                            author = Author(
                                first_name=first_name, middle_names=middle_name, last_name=last_name)
                        else:
                            author = Author(
                                first_name=first_name, last_name=last_name)
            ...
            category = get_object_or_404(
                Category, id=int(request.POST['category']))
            book_type = request.POST['book_type']

            series = None
            if request.POST['series_select'] != 'ignore':
                series = get_object_or_404(
                    Series, id=int(request.POST['series_select']))
...
def del_review(request, review_id):
    ...
    if request.user == review.user:
        ...
        review.delete()
        messages.success(request, 'Your review for ' +
