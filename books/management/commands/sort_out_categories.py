...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("There are " + str(len(queryset)) +
                          " books total to deal with, oh vey!")
        sleep(1)
        ...
        sleep(1)
        queryset_children = Book.objects.filter(category__isnull=True, book_tags__name__in=[
                                                "children's", "middle grade"]).distinct()
        ...
        sleep(1)
        queryset_ya = Book.objects.filter(category__isnull=True, book_tags__name__in=[
                                          "young adult", "teen"]).distinct()
        if queryset_ya:
        ...
        sleep(1)
        queryset_sff = Book.objects.filter(category__isnull=True, book_tags__name__in=[
                                           "sci-fi", "fantasy"]).distinct()
        if queryset_sff:
        ...
        sleep(1)
        queryset_non = Book.objects.filter(category__isnull=True, book_tags__name__in=[
                                           "non-fiction", "memoir"]).distinct()
        if queryset_non:
